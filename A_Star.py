...
class Block:
    def __init__(self, x, y, parent, end_coords):
        ...
        if end_coords:
            self.__H = 4 * min(abs(end_coords[0] - x), abs(end_coords[1] - y)) +\
                       10 * max(abs(end_coords[0] - x), abs(end_coords[1] - y))
        else:
            self.__H = 0

        if parent:
            self.__G = parent.getG() + int(10 * ((parent.getY() - y) ** 2 + (parent.getX() - x) ** 2) ** (1/2))
        else:
            self.__G = 0
    ...
    def setParent(self, parent):
        self.__parent = parent
        self.__G = int(10 * ((parent.getY() - self.getY()) ** 2 + (parent.getX() - self.getX()) ** 2) ** (1/2)) +\
                   parent.getG()
...
class Board(tk.Canvas):
    def __init__(self, master, width, height, block_size):
        tk.Canvas.__init__(self, master, width=width*block_size, height=height*block_size)
        ...
        for i in range(width):
            for j in range(height):
                self.__grid.append(self.create_rectangle(i * block_size + border, j * block_size + border,\
                                                         (i+1) * block_size - border, (j+1) * block_size - border))

    def mark_block(self, coords, state):
        color_dict = {"open": "orange1", "closed": "orange4", "path": "cyan", "obstacle": "gray", "start": "green", "end": "red"}
    ...
    def board_coords(self, canvas_coords):
        return (int(canvas_coords[0] / self.__size), int(canvas_coords[1] / self.__size))

    def canvas_coords(self, board_coords):
        canvas_coords = []
        for coords in board_coords:
            canvas_coords.append((coords[0] * self.__size, coords[1] * self.__size))
        return canvas_coords

    def getDims(self):
        return (int(self.__width / self.__size), int(self.__height / self.__size))
        ...
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                ...
                if (x, y) in open_coords and\
                    ...
                    if block.getG() > current.getG() + int(10 * (dx ** 2 + dy ** 2) ** (1/2)):
